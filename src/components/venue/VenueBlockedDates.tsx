...
export default function VenueBlockedDates({ venueId, venueName }: VenueBlockedDatesProps) {
  ...
  const handleBlockDate = async () => {
    ...
    const success = await blockDate(selectedDate, true, reason);
    if (success) {
      toast({
        title: "Date blocked",
        description: `${format(selectedDate, 'PPP')} has been blocked from bookings.`,
      });
      setReason('');
    } else {
      toast({
  ...
  return (
    <Card className="border border-white/10 bg-findvenue-card-bg">
      <CardHeader>
        ...
        <CardDescription>
          Block dates on your calendar to prevent new bookings. You cannot block dates that already have bookings.
        </CardDescription>
      </CardHeader>
              ...
              {blockedDates.map((blockedDate) => (
                <div key={blockedDate.id} className="flex items-center justify-between p-3">
                  <div>
                    ...
                    {blockedDate.reason && <div className="text-sm text-findvenue-text-muted">{blockedDate.reason}</div>}
                  </div>
                  <Button 
                    variant="ghost" 
                    size="sm"
