...
serve(async (req) => {
  ...
  try {
    ...
    if (type === 'chat') {
      if (!venueInfo) {
        // If it's a general query, fetch some venue recommendations
        const { data: venueRecommendations } = await supabaseClient
          .from('venues')
        ...
        try {
          ...
          if (openaiData.choices && openaiData.choices[0]?.message?.content) {
            ...
            return new Response(
              JSON.stringify({ answer, venues: venueRecommendations }),
              { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          } else {
            throw new Error("Invalid response from OpenAI API");
        ...
        // This is a venue-specific query
        try {
          const OPENAI_API_KEY = Deno.env.get('OPENAI_API_KEY');
          if (!OPENAI_API_KEY) {
            throw new Error("OPENAI_API_KEY not found in environment variables");
